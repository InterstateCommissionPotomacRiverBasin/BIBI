...
run_metrics <- function(long.df, taxa.rank) {
  ...
  select(unique_id) %>% 
  distinct() %>% 
  mutate(GOLD = 100 - (taxa_pct(long.df, unique_id, count, class, c("gastropoda", "oligochaeta")) +
         ...
         PCT_EPT_RICH = taxa_pct_rich(long.df, unique_id, rare_count, order, 
                                      taxon = c("ephemeroptera", "plecoptera", "trichoptera"),
                                      !!taxa.rank),
         RICH_EPHEMEROPTERA = taxa_rich(long.df, unique_id,  rare_count, order, "ephemeroptera", !!taxa.rank),
         PCT_COLLECT = taxa_pct(long.df, unique_id, count, bibi_ffg, c("cg", "cf")),
         PCT_PREDATOR = taxa_pct(long.df, unique_id, count, bibi_ffg, "pr"),
         RICH_FILTER = taxa_rich(long.df, unique_id, rare_count, bibi_ffg, "cf", !!taxa.rank),
         RICH_CLIMB = taxa_rich(long.df, unique_id, rare_count, bibi_habit, "cb", !!taxa.rank),
         ASPT_MOD = taxa_tol_index(long.df, unique_id, count, aspt),
         HBI = taxa_tol_index(long.df, unique_id, count, bibi_tv),
         PCT_EPT_NO_HYDRO = if_else(taxa_pct(long.df, unique_id, count, family, "hydropsychidae") == 0, 0, 
                                    100 * (1 - (taxa_pct(long.df, unique_id, count, family, "hydropsychidae")/ 
           taxa_pct(long.df, unique_id,count, order, c("ephemeroptera", "plecoptera", "trichoptera"))))),
         ...
         RICH_BURROW = taxa_rich(long.df, unique_id, rare_count, bibi_habit, "bu", !!taxa.rank),
         ...
         RICH_TOL = taxa_rich(long.df, unique_id, rare_count, bibi_tv, 7:10, !!taxa.rank),
         PCT_COTE = taxa_pct(long.df, unique_id, count, order, c("coleoptera", "odonata", 
                                                                "trichoptera", "ephemeroptera")),
         PCT_EPT_RICH_NO_TOL = taxa_pct_rich(long.df, unique_id, rare_count, order, 
                                             taxon = c("ephemeroptera", "plecoptera", "trichoptera"),
                                             !!taxa.rank,
                                             exclusion.col = bibi_tv, exclusion.vec = 7:10),
         RICH_TRICHOPTERA = taxa_rich(long.df, unique_id, rare_count, order, "trichoptera", !!taxa.rank),
         RICH_COLLECT = taxa_rich(long.df, unique_id, rare_count, bibi_ffg, c("cg", "cf"), !!taxa.rank),
         PCT_CLING = taxa_pct(long.df, unique_id, count, bibi_habit, "cn"),
         RICH_CLING = taxa_rich(long.df, unique_id, rare_count, bibi_habit, "cn", !!taxa.rank),
         PCT_OLIGO_CHIRO = taxa_pct(long.df, unique_id, count, class, "oligochaeta") +
           taxa_pct(long.df, unique_id, count, family, "chironomidae"),
         RICH_PREDATOR = taxa_rich(long.df, unique_id, rare_count, bibi_ffg, "pr", !!taxa.rank),
...
file.name <- paste0("D:/ZSmith/Projects/Chessie_BIBI/greg_pond/output/bibi_metrics_", Sys.Date(), ".csv")
...
file.name <- paste0("D:/ZSmith/Projects/Chessie_BIBI/greg_pond/output/bibi_scores_", Sys.Date(), ".csv")
...
g.plot <- ggplot(test) + 
  ...
  scale_fill_manual(name = "rating",
                    ...
                    values = c("excellent" = "darkgreen",
                               ...
                               "very_poor" = "red3"))
g.plot
file.name <- paste0("D:/ZSmith/Projects/Chessie_BIBI/greg_pond/output/bibi_stacked_", Sys.Date(), ".png")
