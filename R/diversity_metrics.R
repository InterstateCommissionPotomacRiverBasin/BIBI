...
pct_dom <- function(taxa.wide, dom.level){
  dom_taxa <- function(dom) function(df) order(df, decreasing = TRUE)[dom]
  just.counts <- taxa.wide[, 6:ncol(taxa.wide)]
  n <- colnames(just.counts[(apply(just.counts[1:nrow(just.counts), ], 1, dom_taxa(1)))])
  
  if(dom.level == 1){
    DOM1 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(1)(x)])
    return(DOM1 / rowSums(taxa.wide[, 6:ncol(taxa.wide)]) * 100)
  }

  if(dom.level == 2){
    DOM1 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(1)(x)])
    DOM2 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(2)(x)])
    return((DOM1 + DOM2) / rowSums(taxa.wide[, 6:ncol(taxa.wide)]) * 100)
  }

  if(dom.level == 3){
    DOM1 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(1)(x)])
    DOM2 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(2)(x)])
    DOM3 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(3)(x)])
    return((DOM1 + DOM2 + DOM3) / rowSums(taxa.wide[, 6:ncol(taxa.wide)]) * 100)
  }
  ...
  if(dom.level == 4){
    ...
    DOM2 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(2)(x)])
    DOM3 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(3)(x)])
    DOM4 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(4)(x)])
    return((DOM1 + DOM2 + DOM3 + DOM4) / rowSums(taxa.wide[, 6:ncol(taxa.wide)]) * 100)
  }
  ...
  if(dom.level == 5){
    ...
    DOM3 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(3)(x)])
    DOM4 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(4)(x)])
    DOM5 <- apply(taxa.wide[, 6:ncol(taxa.wide)], 1, function(x) x[dom_taxa(5)(x)])
...
simpsons <- function(taxa.wide) {
  return(vegan::diversity(taxa.wide[, 6:ncol(taxa.wide)], "simpson"))
}
...
shannon <- function(taxa.wide) {
  return(vegan::diversity(taxa.wide[, 6:ncol(taxa.wide)], "shannon"))
}
...
abundance <- function(taxa.wide){
  return(rowSums(taxa.wide[, 6:ncol(taxa.wide)]))
}
