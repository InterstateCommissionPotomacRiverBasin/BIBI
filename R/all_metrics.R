...
all_metrics <- function(master.df, long.df, taxa.rank = "FAMILY", rare = "NON_RARE",
  ...
  if(rare %in% "VEG_RARE") rare.long.df <- prep_rrarefy(long.df, taxa.rank, pct_un)
  if (seed == TRUE) set.seed(64)
  ...
  if(rare %in% c("DIV_RARE", "ALL_RARE", "VEG_RARE")){
    all.div <- all_diversity(master.df, rare.long, rare.class.wide, rare.order.wide, rare.family.wide,
                             rare.tribe.wide, rare.genus.wide, rare.df, taxa.rank)
  }else{
    if(rare %in% "NON_RARE"){
      all.div <- all_diversity(master.df, long.df, class.wide, order.wide, family.wide, tribe.wide, genus.wide,
  ...
  if(rare %in% c("ALL_RARE", "VEG_RARE")){
    all.non_div <- all_non_diversity(master.df, rare.long, rare.class.wide, rare.order.wide,
                                     rare.family.wide, rare.tribe.wide, rare.genus.wide,
                                     rare.df, taxa.rank)
  }else{
    if(rare %in% c("DIV_RARE", "NON_RARE")){
      all.non_div <- all_non_diversity(master.df, long.df, class.wide, order.wide, family.wide,
...
all_diversity <- function(master.df, long.df, class.wide, order.wide, family.wide, tribe.wide, genus.wide,
                          taxa.rank.df, taxa.rank){
  ...
  if(taxa.rank %in% c("FAMILY", "GENUS")){
    ...
    # FFGs
    metrics$RICH_COLLECT <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = c("CG", "CF"), taxa.rank)
    metrics$RICH_GATHER <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = "CG", taxa.rank)
    metrics$RICH_FILTER <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = "CF", taxa.rank)
    metrics$RICH_SHRED <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = "SH", taxa.rank)
    metrics$RICH_SCRAPE <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = "SC", taxa.rank)
    metrics$RICH_PREDATOR <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_FFG", attribute.interest = "PR", taxa.rank)
    # Habits
    metrics$RICH_CLIMB <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "CB", taxa.rank)
    metrics$RICH_SWIM <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "SW", taxa.rank)
    metrics$RICH_CLING <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "CN", taxa.rank)
    metrics$RICH_BURROW <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "BU", taxa.rank)
    metrics$RICH_SPRAWL <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "SP", taxa.rank)
    metrics$RICH_SKATE <- rich_attribute(taxa.rank.df, master.df, attribute.column = "BIBI_HABIT", attribute.interest = "SK", taxa.rank)
  }
...
all_non_diversity <- function(master.df, long.df, class.wide, order.wide, family.wide, tribe.wide, genus.wide,
                              taxa.rank.df, taxa.rank){
  metrics <- data.frame(taxa.rank.df[, c("EVENT_ID", "STATION_ID", "DATE",
                                     "SAMPLE_NUMBER", "AGENCY_CODE")])
  colnames(metrics) <- c("EVENT_ID", "STATION_ID", "DATE",
